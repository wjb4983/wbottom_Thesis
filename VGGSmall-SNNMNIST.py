...
for epoch in range(n_epochs):
    ...
    for step, batch in enumerate(dataloader):#tqdm(dataloader)):
        ...
        if plot:
            ...
            image = batch["image"].view(1,28,28).permute(1, 2, 0)
            image = image / image.max()
            inpt = inputs["X"].view(time, 7*7*128).sum(0).view(7,7,128)
            ...
            inpt_axes, inpt_ims = plot_input(
                image, local_inpy, label=batch["label"], axes=inpt_axes, ims=inpt_ims
            )
