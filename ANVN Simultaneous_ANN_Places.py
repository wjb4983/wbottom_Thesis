...
# Load the Places365 dataset (train and validation sets)
train_dataset = datasets.Places365(
    root='E:\places365',  # Path to download the dataset
    split='train-standard',  # 'train-standard' for training data
    small=True,  # Set to True to download the smaller version (256x256 images)
...
val_dataset = datasets.Places365(
    root='E:\places365',  # Path to download the dataset
    split='val',  # 'val' for validation data
    small=True,  # Set to True to download the smaller version (256x256 images)
    # download=True,  # Download the dataset if not available
    transform=transform
)
...
class Net(nn.Module):
    def __init__(self, reg_strength=0.01, clip_value=1.0):
        ...
        self.fc1 = nn.Linear(3*224*224, num_hidden, bias=False)
        self.fc1_ReLU_scaler = ReLU_Scaler(num_hidden)
        self.fc2 = nn.Linear(num_hidden, 365, bias=False)
...
def validate(loss_vector, accuracy_vector):
    ...
    val_loss /= num_data#len(validation_loader)
    loss_vector.append(val_loss)

    accuracy = 100. * correct.to(torch.float32) / num_data#len(validation_loader.dataset)
