...
lambdas = [0.1,0.25,0.5,1.0,1.75,2.0]
for lam in lambdas:
    SNN_copy = deepcopy(SNN)
    
    ciu_ = ((ciu - min(ciu))/(max(ciu) - min(ciu)))
    ciu_ = 1-np.exp(-lam *ciu_.cpu())
    print(ciu)
    ...
    for i,num in enumerate(ciu_):
        SNN_copy.connections["0","1"].w[:,i] = SNN_copy.connections["0","1"].w[:,i] * num * num
    for index, (data, target) in enumerate(train_loader2):
        if index > 100:
            break
        num_data2 +=1
