...
class Net(nn.Module):
    def __init__(self, reg_strength=0.01, clip_value=1.0):
        ...
        self.fc1 = nn.Linear(1*28*28, num_hidden, bias=False)
...
data=None
SNN = ann_to_snn(model, input_shape=(1,28,28), data=data, percentile=percentile, node_type=ANVN_SRIFNodes)
...
if os.path.exists("hidden_spikes_256.pt") and 0==1:
...
start=1
multipliers = [14,16,18,20,22,24,26,28,30,33,37,40]
...
for multiplier in multipliers:
    ...
    for energy in energies:
        ...
        print("="*30)
        del SNN_copy
results.to_excel(f'ANVNSequentialcheck.xlsx', index=False)