...
def calculate_usefulness(spike_record, correct_neuron_index, total_time):
    """
    Calculate the usefulness of neurons based on their spike contributions
    to the correct classification neuron.

    Parameters:
    - spike_record: A dictionary where keys are neuron indices and values are lists of spike times.
    - correct_neuron_index: Index of the correct classification neuron.
    - total_time: Total simulation time.

    Returns:
    - usefulness_scores: A dictionary where keys are neuron indices and values are usefulness scores between 0 and 1.
    """
    usefulness_scores = {}
    
    # Count total spikes for each neuron
    total_spikes = {neuron: len(spikes) for neuron, spikes in spike_record.items()}
    
    # Count spikes contributing to the correct classification neuron
    correct_spikes = {neuron: 0 for neuron in spike_record.keys()}
    
    # Assume spike_record[correct_neuron_index] gives the spikes of the correct classification neuron
    correct_spike_times = set(spike_record[correct_neuron_index])
    
    for neuron, spikes in spike_record.items():
        for spike_time in spikes:
            if spike_time in correct_spike_times:
                correct_spikes[neuron] += 1
    
    # Calculate usefulness score
    for neuron in spike_record.keys():
        if total_spikes[neuron] > 0:
            usefulness_scores[neuron] = correct_spikes[neuron] / total_spikes[neuron]
        else:
            usefulness_scores[neuron] = 0.0
...
def calculate_intermediate_usefulness(intermediate_spike_record, weight_matrix, correct_neuron_index, total_time):
    ...
    """
    intermediate_spike_record = intermediate_spike_record.reshape(100,1,-1)
...
data=None
SNN = ann_to_snn(model, input_shape=(1,28,28), data=data, percentile=percentile)

print(SNN)
print(SNN.connections["0","1"].w.shape, SNN.connections["2","3"].w.shape, SNN.connections["4","5"].w.shape)
SNN.add_monitor(
...
for index, (data, target) in enumerate(train_loader2):
    ...
    # print(correct)
    if index == 0:
        ciu = calculate_intermediate_usefulness(s['3'], SNN.connections["4","5"].w, target[0], time)
    else:
        ciu += calculate_intermediate_usefulness(s['3'], SNN.connections["4","5"].w, target[0], time)
    spikes_ = {
...
ciu = ciu.reshape(SNN.connections['2','3'].w.shape)
correct2=0
num_data2 = 0
net_spikes2 = 0
# for i,num in enumerate(ciu):
#     SNN.connections["0","1"].w[:,i] = SNN.connections["0","1"].w[:,i] * num
SNN.connections["0","1"].w.copy(SNN.connections["2","3"].w*ciu)
for index, (data, target) in enumerate(train_loader2):
    if index > 250:
        break
    num_data2 +=1
    ...
    # print(correct)
    if index == 0:
        ciu = calculate_intermediate_usefulness(s['1'], SNN.connections["4","5"].w, target[0], time)
    else:
        ciu += calculate_intermediate_usefulness(s['1'], SNN.connections["4","5"].w, target[0], time)
    spikes_ = {
        layer: spikes[layer].get("s")[:].contiguous() for layer in spikes
