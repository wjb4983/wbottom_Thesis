...
#from https://github.com/BasBuller/PySNN/blob/master/examples/mlp_example.py
from pysnn.connection import Linear
from pysnn.neuron import FedeNeuron, Input, LIFNeuron
...
input_dynamics = (dt, alpha_t, tau_t)
neuron_dynamics = (thresh, v_rest, alpha_v, alpha_v, dt, duration_refrac, tau_v, tau_t)
connection_dynamics = (batch_size, dt, delay)

class SNN(SNNNetwork):
    def __init__(self, input_size, num_hidden):
        super(SNN, self).__init__()
        self.input = Input((batch_size, 1, input_size), *input_dynamics)

        # Layer 1
        self.mlp1_c = nn.Linear(input_size, num_hidden, *connection_dynamics)
        self.neuron1 = LIFNeuron((batch_size, 1, num_hidden), *neuron_dynamics)
        # self.neuron1 = LIFNeuron((batch_size, 1, num_hidden), *neuron_dynamics)
        self.add_layer("fc1", self.mlp1_c, self.neuron1)
        
        # Layer 2
        self.mlp2_c = Linear(num_hidden, output_size, *connection_dynamics)
        self.neuron2 = FedeNeuron((batch_size, 1, output_size), *neuron_dynamics)
        # self.neuron2 = FedeNeuron((batch_size, 1, output_size), *neuron_dynamics)
...
model = SNN(input_size, numhidden)
