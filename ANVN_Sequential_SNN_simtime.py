...
results = pd.DataFrame(columns=['Time', 'Max Energy', 'Energy', 'SNN Accuracy', 'Average Spikes'])
times = [2,5,10,20,40,60,80,100]
for time in times:
    energies = [x for x in range(1000,0,-100)]
    start=1
    multipliers = [1,2,6,10,14,18,22,26,30,50]
    ...
    for multiplier in multipliers:
        ...
        for energy in energies:
            ...
            SNN_copy.to("cuda")
            for layer in set(SNN_copy.layers):
                spikes[layer] = Monitor(
                    SNN_copy.layers[layer], state_vars=["s"], time=int(time / 1.0), device=device
                )
                SNN_copy.add_monitor(spikes[layer], name="%s_spikes" % layer)
            SNN_copy.add_monitor(
            ...
            print("="*30)
            del SNN_copy
    results.to_excel(f'ANVNSequential_{time}time.xlsx', index=False)