...
if os.path.exists("ciu_cf10.pt"):
    ciu = torch.load("ciu_cf10.pt")
else:
...
#     SNN.reset_state_variables()
# print("test_accuracy:",float(correct_test)/10000)
print("ciu net: ",ciu)
...
lambdas = [0.01,0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.6,0.7,0.8]
SNN.to('cpu')
...
for lam in lambdas:
    ...
    percentile = torch.kthvalue(ciu.flatten(), int(ciu.numel() * lam)).values.item()
    ind = ciu < percentile
    ciu_= torch.ones(ciu.shape)
    # ciu_ = ((ciu_ - min(ciu_))/(max(ciu_) - min(ciu_)))
    ciu_[ind] = 0
    # ciu_ = ((ciu - -1))/(1--1)
    # ciu_ = 1.0-np.exp(-lam *ciu_.cpu())
    # ciu_ = 1.2*np.exp(lam*(ciu_.cpu()))/2.7-1
    # ciu_ = np.exp((ciu_.cpu()))/2.7-1
    ciu_[ind] = 0
    # ciu_x = ciu.cpu().flatten().numpy()
    # ciu_y = (ciu_).cpu().flatten().numpy()
    # plt.scatter(ciu_x, ciu_y)
    # print(ciu)
