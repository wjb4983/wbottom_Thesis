...
class ANVN_SRIFNodes(SubtractiveResetIFNodes):
    def __init__(self, *args, spike_limit=99999999, device='cuda', batch_size=batch_size, **kwargs):
    ...
    for me in max_energies: 
        ...
        for energy in energies:
            ...
            if energy != 0:
                ...
                SNN_copy.layers['1'].thresh = (SNN_copy.layers['1'].thresh/2).unsqueeze(0).repeat(batch_size,1)
            ...
            if energy != 0:
                ...
                # counts, bins = np.histogram(tree_output, 30)
                # plt.stairs(counts, bins)
                SNN_copy.layers['1'].set_non_hidden()
                SNN_copy.layers['3'].set_non_hidden()
                # tree_output = torch.tensor(tree_output, device=device)
                two = [2]*len(tree_output)
                for i,num in enumerate(two):
                    SNN_copy.connections["0","1"].w[:,i] = SNN_copy.connections["0","1"].w[:,i] * num
                # SNN_copy.connections["0", "1"].w  = torch.nn.Parameter(torch.tensor(SNN_copy.connections['0','1'].w * maxx -torch.tensor(tree_output, device = device), dtype=torch.float))
                # SNN_copy.connections["0", "1"].w.requires_grad = False
                # SNN_copy.connections["0", "1"].w  = torch.nn.Parameter(torch.tensor(SNN_copy.connections['0','1'].w * maxx -torch.tensor(tree_output, device = device), dtype=torch.float))
